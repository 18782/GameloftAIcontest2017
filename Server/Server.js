...
//+--------------------+------------+---------+-----------+-----------+-------------------------+-------------------------+
//+ COMMAND_SEND_STAGE | Game state |SeverTime| Stack p12 | Point p12 |		Game board     	    | P12 Action & Direction  |
//+--------------------+------------+---------+-----------+-----------+-------------------------+-------------------------+
...
var replayFilename = "";
//serverside
var playerAName="bot1";
var playerBName="bot2";
for (var i=0; i<process.argv.length; i++) {
	if (process.argv[i] == "-p") {
		...
		playerAName = process.argv[i + 1];
		playerBName = process.argv[i + 2];
	}
}
...
function GenerateMap() {
	...
	for (var i=0; i<NUM_BLOCK_OBSTACLE; i++) {
	...
	//g_playerPos[PLAYER_2] = new Position (2, 0);
...
function GameUpdate (){
	...
	{
		var posAction = MoveChecker(PLAYER_1, g_playerActionDir[PLAYER_1]);
	...
	{
		var posAction = MoveChecker(PLAYER_2, g_playerActionDir[PLAYER_2]);
		if(posAction != null 
	...
	else if(g_playerAction[PLAYER_1] == ACTION_PICK && g_playerStack[PLAYER_1] < PLAYER_STACK_SIZE){
		//update pick
		var posSoftBlock = PickCheck(PLAYER_1, g_playerActionDir[PLAYER_1], g_map_p1);
	...
	else if(g_playerAction[PLAYER_2] == ACTION_PICK && g_playerStack[PLAYER_2] < PLAYER_STACK_SIZE){
		//update pick
		var posSoftBlock = PickCheck(PLAYER_2, g_playerActionDir[PLAYER_2], g_map_p2);
		if(posSoftBlock != null)
		{
			g_playerStack[PLAYER_2]++;
...
function PickCheck(player, dir, g_map_){
	if (dir == DIRECTION_LEFT) {
		if (g_playerPos[player].x > 0 
...
var server = ws.createServer(function (socket) {
    ...
    socket.on("text", function (data) {
		...
		else if( command == COMMAND_GET_NAME){
			socket.sendText("N"+playerAName+"|"+playerBName);
		}
    });
...
function Broadcast () {
	...
	data += String.fromCharCode(g_gameTimeLoop);
